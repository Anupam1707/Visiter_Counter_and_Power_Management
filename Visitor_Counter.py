...
totalf = "total"
totalinf = "totalin"
with open(f"{totalf}.txt", "w") as f:
	pass
with open(f"{totalinf}.txt", "w") as f:
	pass
...
def parse_arguments():
    ...
    ap.add_argument("-i", "--input", type=str, default="test_loop.mp4",
...
def people_counter():
	...
	while True:
		...
		if key == ord("q"):
			...
			if peoplein > 0 and totalpeople > 0:
				peoplein -= 1
				totalpeople -= 1
		elif key== ord("r"):
			totalpeople += 1
			...
			peoplein = 0
		with open(f"{totalf}.txt", "w") as f:
			f.write(str(totalpeople))
		with open(f"{totalinf}.txt", "w") as f:
			f.write(str(peoplein))
		try:
			if peoplein > 0:
				board.digital[light].write(1)
			else:
